# 📚 Rikta Framework Documentation

Welcome to the official documentation for Rikta Framework.

## 🚀 Getting Started

- [Installation & Quick Start](../README.md)
- [Example Application](../example/README.md)

## 📖 Guide

Core concepts and features of the framework.

- [Architecture](./guide/architecture.md) - How Rikta works under the hood
- [Dependency Injection](./guide/dependency-injection.md) - The DI container and `@Autowired`
- [Routing](./guide/routing.md) - Controllers, routes, and parameters
- [Validation](./guide/validation.md) - Type-safe validation with Zod
- [Lifecycle](./guide/lifecycle.md) - Hooks and events
- [Error Handling](./guide/error-handling.md) - Exceptions and filters

## 🔧 API Reference

Detailed API documentation.

- [Decorators](./api/decorators.md) - Complete list of decorators
- [Testing](../tests/README.md) - Testing utilities and patterns

## 🤝 Contributing

- [Contribution Guide](../CONTRIBUTING.md) (Coming soon)
